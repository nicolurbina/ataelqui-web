...
async function checkInventory(productId) {
    ...
    const snapshot = await inventoryRef.where('productId', '==', productId).get();
    ...
    if (snapshot.empty) {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            console.log('Inventory Entry:', doc.id, '=>', data);
            total += (data.quantity || 0);
        });
        console.log(`Total Calculated Inventory: ${total}`);
    }
}

// SKU from screenshot
checkProduct('7506195144947');
